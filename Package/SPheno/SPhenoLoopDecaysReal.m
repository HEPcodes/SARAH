...
pos=Position[VerticesInv[All],C[e1,p1,p2]];
If[pos==={},Vertex::DoesNotExist="The Vertex `` does not exist!";
Message[Vertex::DoesNotExist,C[e1,p1,p2]];
Abort[];,pos=pos[[1,1]];];
toordered=VerticesOrg[All][[pos]]/.A_[{b__}]->A;
ctvlist={ct1,ct2,ct3};
cinlist=Array[Symbol["cin"<>ToString[#]]&,{3}];
cttarlist=Table[{ps[[i]],cinlist[[i]]},{i,1,3}];
ctrepr={};
For[i=1,i<=3,i++,tpos=Position[cttarlist,toordered[[i]]][[1,1]];
...
WriteRealCorrections[NameRoutine_,DecayingParticle_,twobodydecays_,OneLtbds_,type_]:=Block[{i,j,k,couplings,masses,p1,p2,ps,ind,coup,dim,dimI,treecolfac,Casarr,Chargearr,Chargevec,chargepresent,Casvec,chargeeasy,colourpresent,pCas,pDyn,routine},
...
p1=OneLtbds[[1]];
p2=OneLtbds[[2]];
ps={AntiField[DecayingParticle],p1,p2};
...
If[chargepresent,Chargearr=Flatten[Table[Chargevec[[i]]*Chargevec[[j]],{i,1,3},{j,i,3}]];];
...
FFV,
	...
	(*pDyn=SA`DynL[DecayingParticle,3];*)
		...
		WriteString[outputfortran,"  Call hardgluonFFZW(Mex1,Mex2,Mex3,MLambda,"<>ToString[FortranForm[If[Head[pCas]===Integer,1.0*pCas,pCas]]]<>",g3,CoupL,CoupR,Gammarealgluon"<>dimI<>",kont)\n"];
		];(* if pDyn\[Equal]0*)
...
If[!colourpresent,
WriteString[outputfortran," GammarealGluon"<>dimI<>" = 0._dp \n"];,
WriteString[outputfortran," Call hardradiationSSV(Mex1,Mex2,Mex3,MLambda,em,1._dp,1._dp,0._dp,Coup,Gammarealgluon"<>dimI<>",kont)\n"];
WriteString[outputfortran," GammarealGluon"<>dimI<>" = GammarealGluon"<>dimI<> "*"<>ToString[FortranForm[pCas]]<>" \n"];
];
, (* End SSV *)

SVV,
WriteString[outputfortran,"! SVV Bremsstrahlung not yet implemented\n"];
WriteString[outputfortran,"  GammarealGluon"<>dimI<>" = 0._dp \n"];
WriteString[outputfortran,"  GammarealPhoton"<>dimI<>" = 0._dp \n\n"];
